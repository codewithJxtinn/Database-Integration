...
A REST API that connects a Node.js/Express backend to a SQLite database,
implementing full CRUD, schema design, and SQL-injection-safe queries.

## Stack
- **Runtime:** Node.js (v22+, uses the built-in `node:sqlite` module — no
  native compilation, no external DB server to install)
- **Framework:** Express
...
node test.js
```
This boots the app in-process and walks through every CRUD operation,
plus the constraint/security checks (duplicate email, invalid status,
missing foreign key, cascade delete).
...
```
One user has many tasks (1:Many). `ON DELETE CASCADE` means deleting a
user also removes their tasks.
...
- **Every** query uses parameterized placeholders (`?`) via
  `db.prepare(...).run(...)` — user input is never concatenated into
  SQL strings, which is the standard defense against SQL injection.
